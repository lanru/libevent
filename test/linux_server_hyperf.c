...
void
error_cb(struct bufferevent *bev, short event, void *arg)
{
	evutil_socket_t fd = bufferevent_getfd(bev);
	printf("fd = %u, ", fd);
...
//回调函数，用于监听连接进来的客户端socket
void
do_accept_buffer(evutil_socket_t fd, short event, void *arg)
{
	int client_socketfd;			//客户端套接字
	struct sockaddr_in client_addr; //客户端网络地址结构体
	int in_size = sizeof(struct sockaddr_in);
	...
	// struct event *ev;
	// ev = event_new(base_ev, client_socketfd, EV_TIMEOUT|EV_READ|EV_PERSIST,
	// do_read, base_ev); event_add(ev, NULL);

	//创建一个evbuffer，用来缓冲客户端传递过来的数据
	struct evbuffer *buf = evbuffer_new();
	//创建一个bufferevent
	struct bufferevent *bev =
		bufferevent_socket_new(base_ev, client_socketfd, BEV_OPT_CLOSE_ON_FREE);
...
static void
discard_cb(int severity, const char *msg)
{
...
static void
write_to_file_cb(int severity, const char *msg)
{
	const char *s;
	if (!logfile)
		return;
...
{
	FILE *fp = NULL; /* 需要注意 */
	fp = fopen("/home/wlh/1.txt", "r+");
	if (errno){
		perror("出错了:");
	}
	set_logfile(fp);
	write_to_file_cb(_EVENT_LOG_WARN,"test123456");
}
